...
class TestLogInPageComponent extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            userDetails: "",
            testuserdetails: JSON.parse(localStorage.getItem("cred")),
    ...
    testonFormSubmit(event){
        event.preventDefault();
    
        if(this.props.userName === this.state.testuserdetails[0]){
            if(this.props.password === this.state.testuserdetails[1]){
                this.props.history.push(`/About/`);
                console.log("success");
                alert("success");
